...
public:
  ...
  Homography            (const vector<Vector2d>& _fts1,
                         const vector<Vector2d>& _fts2,
  ...
  const vector<Vector2d>& fts_c1; //!< Features on first image on unit plane
  const vector<Vector2d>& fts_c2; //!< Features on second image on unit plane
  vector<bool> inliers;
